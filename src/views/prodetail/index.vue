  ...
  <div class="prodetail">
    ...
    <van-swipe :autoplay="3000" @change="onChange">
      <van-swipe-item v-for="(image, index) in images" :key="index">
        <img :src="image" v-lazy="image.external_url" />
      </van-swipe-item>
    ...
    <div class="footer">
      ...
      <div class="icon-cart">
        <span v-if="cartTotal > 0" class="num" @click="$router.push('/cart')">{{ cartTotal }}</span>
        <van-icon name="shopping-cart-o" />
        <span>购物车</span>
      </div>
      <div class="btn-add" @click="addFn">加入购物车</div>
      <div class="btn-buy" @click="toBuy" >立刻购买</div>
    </div>
...
export default {
  ...
  methods: {
    ...
    addFn () {
      console.log('是否未登录：', this.isLogin())
      if (this.isLogin()) {
        console.log('未登录')
        return
      }
    ...
    async addCart () {
      this.tankuang()
      const { data } = await addCarts(this.goodsId, this.addCount, this.detail.skuList[0].goods_sku_id)
